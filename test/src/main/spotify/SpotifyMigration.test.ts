...
describe('Drizzle Migration 0020 (spotify_playlist_links Live PGlite Execution & Schema Parity)', () => {
  ...
  const journalPath = path.resolve(__dirname, '../../../../resources/drizzle/meta/_journal.json');
  ...
  it('should execute 0020 migration on fresh PGlite database and verify full schema, constraints, defaults, and cascade parity', async () => {
    ...
    const runtimeColumns = new Map(columnsResult.rows.map((r) => [r.column_name, r]));
    ...
    for (const expected of expectedColumnDefinitions) {
      ...
      expect(
        col,
        `Expected column ${expected.name} to exist in spotify_playlist_links`
      ).toBeDefined();
