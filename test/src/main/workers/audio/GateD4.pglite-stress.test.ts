...
describe('Gate D4.1: Real PGlite Concurrency Stress & Transaction Rollback', () => {
  ...
  it('proves real PGlite rolls back Track 1 update when Track 2 fails conditional update (Zero Partial Persistence)', async () => {
    ...
    await fs.writeFile(
      path.join(
        tempUserDataDir,
        'loudness_blocks',
        `${s1.id}_v${CURRENT_REPLAYGAIN_GENERATOR_VERSION}.bin`
      ),
    ...
    await fs.writeFile(
      path.join(
        tempUserDataDir,
        'loudness_blocks',
        `${s2.id}_v${CURRENT_REPLAYGAIN_GENERATOR_VERSION}.bin`
      ),
    ...
    await fs.writeFile(
      path.join(
        tempUserDataDir,
        'loudness_blocks',
        `${s3.id}_v${CURRENT_REPLAYGAIN_GENERATOR_VERSION}.bin`
      ),
    ...
    testDb.transaction = (async (callback: (trx: unknown) => Promise<unknown>) => {
      return originalTransaction(async (trx: unknown) => {
        const typedTrx = trx as {
          query: {
            replayGain: { findMany: (...args: unknown[]) => Promise<{ songId: number }[]> };
          };
          update: unknown;
        };
        const originalTrxFindMany = typedTrx.query.replayGain.findMany.bind(
          typedTrx.query.replayGain
        );
