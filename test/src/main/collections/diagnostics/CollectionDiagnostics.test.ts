...
describe('CollectionDiagnostics', () => {
  ...
  it('should find empty folders', async () => {
    const [emptyFolder] = await db.insert(playlists).values({ name: 'Empty', playlistType: 'folder' }).returning({ id: playlists.id });
    const [fullFolder] = await db.insert(playlists).values({ name: 'Full', playlistType: 'folder' }).returning({ id: playlists.id });
    
    // Add child to fullFolder
    await db.insert(playlists).values({ name: 'Child', parentId: fullFolder.id, playlistType: 'standard' });
  ...
  it('should find empty playlists', async () => {
    const [emptyPlaylist] = await db.insert(playlists).values({ name: 'Empty', playlistType: 'standard', itemCount: 0 }).returning({ id: playlists.id });
    const [fullPlaylist] = await db.insert(playlists).values({ name: 'Full', playlistType: 'standard', itemCount: 5 }).returning({ id: playlists.id });
    const [emptySmart] = await db.insert(playlists).values({ name: 'SmartEmpty', playlistType: 'smart', itemCount: 0 }).returning({ id: playlists.id });
  ...
  it('should find duplicate names', async () => {
    const [root1] = await db.insert(playlists).values({ name: 'DuplicateName', playlistType: 'standard' }).returning({ id: playlists.id });
    const [root2] = await db.insert(playlists).values({ name: 'DuplicateName', playlistType: 'folder' }).returning({ id: playlists.id });
