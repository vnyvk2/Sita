...
describe('songWorkerPool', () => {
  ...
  it('should parse songs with bounded concurrency and queue artwork jobs', async () => {
    const songs = [{ songPath: '/test/1.mp3' }, { songPath: '/test/2.mp3' }];

    (tryToParseSong as Mock)
    ...
    const artworkJobs = vi
      .mocked(libraryScheduler.enqueue)
      .mock.calls.map(([job]) => job)
  ...
  it('should fallback to local parsing for ONLY remaining uncommitted songs when worker fails mid-stream', async () => {
    ...
    vi.mocked(mediaWorkerBridge.parseTrackBatchStream).mockImplementation(
      async (_tracks, options) => {
    ...
    expect(tryToParseSong).not.toHaveBeenCalledWith(
      '/test/1.mp3',
      expect.anything(),
      expect.anything(),
      expect.anything()
  ...
  it('isolates staged transaction state and emits zero ghost jobs or duplicate fallback counts on db.transaction rollback', async () => {
    ...
    vi.mocked(db.transaction).mockRejectedValueOnce(
      new Error('Postgres transaction rollback error')
    );
