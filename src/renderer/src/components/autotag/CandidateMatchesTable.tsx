...
export const CandidateMatchesTable: React.FC<CandidateMatchesTableProps> = ({
  ...
  if (candidates.length === 0 && !loading) {
    return (
      <div className="p-6 text-center bg-background-color-2/20 rounded-xl border border-dashed border-background-color-3/40 text-font-color-dimmed text-sm">
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="border border-background-color-2 rounded-xl overflow-y-auto max-h-[250px] bg-background-color-2/20">
        <table className="w-full border-collapse text-left text-sm">
          <thead className="sticky top-0 z-[2] bg-background-color-2 border-b border-background-color-3/30 text-font-color-dimmed text-xs uppercase tracking-wider">
            <tr>
            ...
            {candidates.map((cand, idx) => {
              ...
              return (
                <tr
                  ...
                  className={`border-b border-background-color-2/50 cursor-pointer transition-colors ${
                    isSelected ? 'bg-background-color-3/20' : 'hover:bg-background-color-2/50'
                  }`}
                >
                  {/* Radio / Loading Indicator */}
                  <td className="px-2.5 py-2 text-center">
                    {isLoadingThis ? (
                      <div className="w-3 h-3 rounded-full border-2 border-font-color-highlight border-t-transparent animate-spin mx-auto" />
                    ) : (
                      <div
                        className={`w-3.5 h-3.5 rounded-full border-2 mx-auto transition-all ${
                          isSelected
                            ? 'border-font-color-highlight bg-font-color-highlight'
                            : 'border-font-color-dimmed/60 bg-transparent'
                        }`}
                      />
                    )}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <div className={`text-sm ${isSelected ? 'font-bold text-font-color-highlight' : 'font-semibold text-font-color'}`}>
                      {cand.title}
                    </div>
                    <div className="text-xs text-font-color-dimmed mt-0.5">
                      {cand.trackCount ? `${cand.trackCount} tracks` : 'Official Release'} {cand.releaseType ? `· ${cand.releaseType}` : ''}
                    </div>
                  </td>

                  {/* Artist */}
                  <td className="px-3 py-2 text-font-color text-xs font-medium">
                    {cand.artist || '—'}
                  </td>

                  {/* Year */}
                  <td className="px-3 py-2 text-font-color-dimmed text-xs">
                    {cand.year ?? '—'}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1.5">
                      {isLoadingThis ? (
                        <span className="text-[0.7rem] font-semibold px-1.5 py-0.5 rounded bg-background-color-3/30 text-font-color-highlight border border-background-color-3/50">
                          Resolving...
                        </span>
                      ) : (
                        <>
                          ...
                          {cand.rankingScore !== undefined && (
                            <span className="text-[0.72rem] font-medium text-font-color-dimmed">
                    ...
                    <div className="text-[0.7rem] text-font-color-dimmed mt-0.5 font-medium">
                      {providerLabel}
                    </div>
                  </td>
                </tr>
