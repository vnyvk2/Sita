...
export const SelectedReleasePanel: React.FC<SelectedReleasePanelProps> = ({
  ...
  return (
    <div className="bg-background-color-2/40 rounded-xl border border-background-color-2 p-5 grid grid-cols-[130px_1fr] gap-5 items-center">
      {/* Left: Artwork & Source Selector */}
      <div className="flex flex-col gap-2 items-center">
        <div className="w-[110px] h-[110px] rounded-lg overflow-hidden bg-background-color-2 border border-background-color-3/40 flex items-center justify-center relative shadow-sm">
          {artworkUrl && artworkSource !== 'local' ? (
            ...
            <div className="flex flex-col items-center gap-1 text-font-color-dimmed">
        ...
        <label className="flex items-center gap-1.5 text-xs text-font-color font-medium cursor-pointer">
          <input
            type="checkbox"
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="flex justify-between items-start">
          <div>
            <div className="text-xl font-bold text-font-color-highlight leading-tight">
              {preview.album.title}
            </div>
            <div className="text-sm text-font-color-dimmed mt-1 font-medium">
              <span className="text-font-color font-semibold">{preview.album.artist}</span> {preview.album.year ? `· ${preview.album.year}` : ''} · {preview.matches.length} tracks
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2 items-center text-xs">
          <div className="bg-background-color-2 border border-background-color-3/40 rounded px-2.5 py-0.5 text-font-color-dimmed font-medium">
            Provider: <span className="text-font-color font-semibold">{getProviderDisplayName(preview.provider)}</span>
          </div>

          <div className="bg-background-color-2 border border-background-color-3/40 rounded px-2.5 py-0.5 text-font-color-dimmed font-medium">
            MBID: <span className="font-mono text-font-color font-semibold">{mbid.slice(0, 18)}...</span>
          </div>
        ...
        {replaceArtwork && (
          <div className="flex items-center gap-3.5 mt-0.5 text-xs text-font-color-dimmed">
            <span className="font-semibold text-font-color">Artwork Source:</span>
            <label className={`flex items-center gap-1.5 cursor-pointer ${artworkSource === 'musicbrainz' ? 'text-font-color font-semibold' : 'text-font-color-dimmed'}`}>
              <input
                type="radio"
                name="artworkSource"
                value="musicbrainz"
            ...
            <label className={`flex items-center gap-1.5 cursor-pointer ${artworkSource === 'coverartarchive' ? 'text-font-color font-semibold' : 'text-font-color-dimmed'}`}>
              <input
                type="radio"
                name="artworkSource"
            ...
            <label className={`flex items-center gap-1.5 cursor-pointer ${artworkSource === 'local' ? 'text-font-color font-semibold' : 'text-font-color-dimmed'}`}>
              <input
                type="radio"
                name="artworkSource"
                value="local"
