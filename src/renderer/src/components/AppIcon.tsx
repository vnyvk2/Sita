...
export interface AppIconProps {
  /** 1-based index (1 to appIcons.length) */
  index: number;
  className?: string;
  alt?: string;
}
...
/**
 * Reusable App Icon component for Nora Allows rendering any of the customizable icons with
...
const AppIcon = memo(function AppIcon({ index, className = 'w-10 h-10', alt }: AppIconProps) {
  const clampedIndex = Math.max(1, Math.min(appIcons.length, index)) - 1;
