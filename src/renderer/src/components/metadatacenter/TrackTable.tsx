...
export const TrackTable: React.FC<TrackTableProps> = ({
  ...
  const perfectMatches = matches.filter(
    (m) => !m.fieldDiffs?.some((d) => d.status === 'changed' || d.status === 'new')
  );
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        >
          <span
            ...
            style={{ fontSize: '16px', color: 'var(--text-color-highlight)' }}
          >
            queue_music
          </span>
        ...
        >
          ...
          <tbody>
            {filteredMatches.map((m, idx) => {
              ...
              const hasDiff = m.fieldDiffs?.some(
              ...
              return (
                <React.Fragment key={m.localSongId || idx}>
                  ...
                  >
                    ...
                    <td
                      style={{ padding: '12px 14px', fontWeight: 600, color: 'var(--text-color)' }}
                    >
                      ...
                      <td colSpan={6} style={{ padding: '14px 18px' }}>
                        ...
                        >
                          {m.fieldDiffs?.map((fd) => (
                            ...
                            >
                              <span style={{ color: 'var(--text-color-dimmed)', fontWeight: 600 }}>
                                {fd.fieldName}:
                              </span>{' '}
                              <span
                                style={{
                                  color: fd.status === 'changed' ? '#34D399' : 'var(--text-color)',
                                  fontWeight: fd.status === 'changed' ? 600 : 400
                                }}
                              >
                                {fd.suggestedValue !== undefined
                                  ? String(fd.suggestedValue)
                                  : String(fd.oldValue ?? '—')}
                              </span>
                            </div>
                          ))}
