...
export class TitleMatchStrategy implements PlaylistRepairStrategy {
  ...
  evaluate(entry: LibraryResolvedPlaylistEntry, candidate: LibrarySongRecord): RepairCandidate | null {
    ...
    const cleanTargetTitle = nameWithoutExt.replace(/^\d+[\s._-]+/, '').trim().toLowerCase();
    ...
    if (cleanTargetTitle === candidateTitle || candidate.path.toLowerCase().includes(cleanTargetTitle)) {
      return {
        song: candidate,
        confidence: 80,
