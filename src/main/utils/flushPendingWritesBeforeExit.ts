...
/**
 * Flushes pending lyrics and metadata writes for the given song path before
 * the current process terminates (relaunch, restart or exit).
 *
 * Any application restart path MUST await this helper before exiting,
 * otherwise pending file writes are killed mid-flight and can leave
 * half-written tags on disk.
 *
 * Flush errors are logged but never propagated: a failing flush must not
