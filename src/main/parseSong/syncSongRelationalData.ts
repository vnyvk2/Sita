...
/**
 * Relational projection of a known-source metadata save: artworks <-> song/album links, song
 * artists, album linkage (+ artist fan-out), genres - including safe cascade deletion of
 * zero-reference entities.
 *
 * Extracted verbatim from updateSongId3Tags (P1 of pipeline unification): behaviorally identical to
...
export const syncSongRelationalData = async ({
  ...
  if (tags.artists) {
    ...
    const unlinkedArtistIds = currentArtistIds.filter(
      (id: number) => !artistsWithIds.some((a) => Number(a.artistId) === id)
    );
  ...
  if (tags.genres) {
    ...
    const unlinkedGenreIds = currentGenreIds.filter(
      (id: number) => !genresWithIds.some((g) => Number(g.genreId) === id)
    );
