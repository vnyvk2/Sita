...
export const linkSongToArtist = async (
  ...
  return trx.insert(artistsSongs).values({ artistId, songId }).onConflictDoNothing().returning();
};

/**
 * Unlinks a song from an artist by deleting the relationship in the artistsSongs table. This does
 * NOT delete the artist or song themselves - only the relationship between them.
 *
 ...
 * **Cascade Behavior:**
 *
 * - All entries in `artistsSongs` linking this artist to songs are automatically deleted (ON DELETE
 *   CASCADE)
 * - All entries in `albumsArtists` linking this artist to albums are automatically deleted (ON DELETE
 *   CASCADE)
 * - All entries in `artistsArtworks` linking this artist to artworks are automatically deleted (ON
 *   DELETE CASCADE)
 *
 * **Important:** This function should only be called after verifying the artist has no remaining
 * songs. Use `getArtistSongIds()` to check before deletion to prevent data inconsistency.
 *
