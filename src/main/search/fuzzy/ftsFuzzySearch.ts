 ...
 *
 * 1. Candidate pool = rows sharing at least one trigram with the query, found via FTS5 trigram
 *    OR-queries (the same candidate model pg_trgm's GIN index uses, including word-boundary
 *    trigrams where expressible).
 * 2. Candidates scored in JS with pg_trgm's exact algorithm; >= 0.3 kept, best first.
 *
 * Returns { id, text } of similar rows (best first), excluding `excludeIds`, so engines can compute
 * match tiers from the display text exactly like the pg path did.
 */
...
export interface FuzzySearchArgs {
  ...
  /**
   * Kept for API compatibility: rawAll targets the same connection, so queries issued here always
...
export const fuzzySearch = async (
  ...
  const rows = await rawAll<{ id: number; text: string }>(
    sql`
  ...
  const threshold =
    bestSim >= PG_SIMILARITY_THRESHOLD ? PG_SIMILARITY_THRESHOLD : PG_SIMILARITY_FLOOR;
