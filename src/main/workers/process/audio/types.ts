...
export interface DecodeChunk {
  /**
   * Normalized audio samples in range [-1.0, 1.0] per channel. channelData[0] is Left (or Mono),
   * channelData[1] is Right, etc.
   */
...
export interface DecodeStreamOptions {
  ...
  /**
   * Target frame count per chunk to bound memory usage. Default is typically 16,384 frames (approx.
   * 64KB per channel buffer).
   */
  chunkSize?: number;
}
