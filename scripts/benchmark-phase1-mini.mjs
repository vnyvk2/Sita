...
async function runPhase1MiniPlayerBenchmark() {
  console.log(`\n=================================================================`);
  console.log(` PHASE 1 MINIPLAYER EMPIRICAL BENCHMARK (Nora-oexxx)`);
  console.log(` Capturing the 7 Decision-Point Metrics After Phase 1 Route/Query Gating`);
  console.log(`=================================================================\n`);
  ...
  const outPath = path.join(rootDir, 'phase1_mini_report.json');
  fs.writeFileSync(outPath, JSON.stringify(phase1Results, null, 2), 'utf8');
  console.log(`\n[Runner] Phase 1 report saved to ${outPath}\n`);
}

runPhase1MiniPlayerBenchmark();
